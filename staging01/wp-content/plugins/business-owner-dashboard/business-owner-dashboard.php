...
function bod_run_db_upgrades() {
    ...
    // v1.2 — add region to owners table (was in schema but may be missing from pre-existing DBs)
    if ( ! get_option( 'bod_db_upgrade_region' ) ) {
        $table = BOD_TABLE_OWNERS;
        $col   = $wpdb->get_results( "SHOW COLUMNS FROM `{$table}` LIKE 'region'" );
        if ( empty( $col ) ) {
            $wpdb->query( "ALTER TABLE `{$table}` ADD COLUMN `region` varchar(100) DEFAULT NULL AFTER `suburb`" );
        }
        update_option( 'bod_db_upgrade_region', true );
    }
}
...
function bod_init_stripe() {
    if (!class_exists('\Stripe\Stripe')) {
        require_once BOD_PLUGIN_DIR . 'stripe-php/init.php';
    }

    if (class_exists('\Stripe\Stripe')) {
        \Stripe\Stripe::setCABundlePath(BOD_PLUGIN_DIR . 'cacert.pem');
    }
