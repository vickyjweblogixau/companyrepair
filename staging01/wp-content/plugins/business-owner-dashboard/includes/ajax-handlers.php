...
add_action('wp_ajax_bod_approve_owner', function() {
    ...
    $owner_id = absint($_POST['owner_id'] ?? 0);
     error_log($label . ': ' . print_r($_POST, true));
...
function bod_ajax_initiate_signup() {
    // Use wp_verify_nonce instead of check_ajax_referer so failures return
    // clean JSON rather than dying with HTTP 403 (which jQuery treats as a
    // network/connection error and shows the generic "Connection error" message).
    if ( ! wp_verify_nonce( sanitize_text_field( $_POST['nonce'] ?? '' ), 'bod_signup' ) ) {
    ...
    try {
        ...
        // Block duplicate emails before creating a Stripe session
        $existing = bod_get_owner_by_email($owner_data['email']);
        if ($existing) {
            $status = $existing->approval_status;
            if ($status === 'approved' && $existing->account_status === 'active') {
                wp_send_json_error(['message' => 'An account with this email already exists. Please log in instead.']);
            } elseif ($status === 'pending') {
                wp_send_json_error(['message' => 'This email is already submitted and pending review. Please check your inbox.']);
            } elseif ($status === 'rejected') {
                wp_send_json_error(['message' => 'This email was previously rejected. Please contact us.']);
            } else {
                wp_send_json_error(['message' => 'This email is already registered. Please contact us if you need help.']);
            }
        }
        ...
        // Top-level safety net — catches both PHP Errors and Exceptions (never HTTP 500)
        error_log('[BOD Signup] Unhandled error in bod_ajax_initiate_signup: ' . get_class($e) . ': ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
...
add_action( 'wp_ajax_bod_validate_email',        'bod_ajax_validate_email' );
function bod_ajax_validate_email() {
     // Verify nonce if provided
    if ( ! empty( $_POST['nonce'] ) ) {
        if ( ! wp_verify_nonce( sanitize_text_field( $_POST['nonce'] ), 'bod_validate_email' ) ) {
    ...
    // Email exists — return specific message based on status
    $status = $existing->approval_status;
    if ( $status === 'approved' && $existing->account_status === 'active' ) {
        wp_send_json_error( [
            'message' => 'An account with this email already exists. <a href="' . home_url( '/business-owner-login/' ) . '">Log in here</a>.',
        ] );
    } elseif ( $status === 'approved' ) {
        wp_send_json_error( [
            'message' => 'This email is registered and pending account setup. Check your inbox for credentials.',
        ] );
    } elseif ( $status === 'pending' ) {
        wp_send_json_error( [
            'message' => 'This email has already been submitted and is pending admin review. Check your inbox.',
        ] );
    } elseif ( $status === 'rejected' ) {
        wp_send_json_error( [
            'message' => 'This email was previously rejected. Please contact us for assistance.',
        ] );
    } else {
        wp_send_json_error( [
            'message' => 'This email is already registered. Please contact us if you need help.',
        ] );
    }
}