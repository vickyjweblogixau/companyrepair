...
function bod_create_signup_checkout($owner_data) {
    ...
    try {
        ...
        error_log('[BOD Stripe] Checkout error: ' . $e->getMessage());
...
function bod_webhook_process_signup($session) {
    ...
    // AUTO-CREATE ACCOUNT + SEND CREDENTIALS (core flow)
    $fresh_owner = bod_get_owner($owner_id);
    if (!$fresh_owner->wp_user_id) {
        $account = bod_create_user_account($owner_id);
        if ($account['success']) {
            error_log('[BOD Webhook] Account created for: ' . $email);
            // Send invoice + credentials email
            bod_send_invoice_and_credentials_email($owner_id, $session);
        } else {
            error_log('[BOD Webhook] Account creation failed: ' . $account['error']);
        }
    } else {
...
function bod_webhook_process_boost($session) {
    ...
    $boost_type = sanitize_key($session->metadata->boost_type ?? '');
    if (!$owner_id || !$boost_type) return;

    // Mark the most-recent active listing for this owner as boosted
    $listing = $wpdb->get_row($wpdb->prepare(
        "SELECT * FROM " . BOD_TABLE_LISTINGS . " WHERE owner_id = %d AND listing_status = 'active' ORDER BY listed_at DESC LIMIT 1",
        $owner_id
    ));

    if ($listing) {
        $wpdb->update(BOD_TABLE_LISTINGS, [
            'active_boost'           => $boost_type,
            'boost_payment_intent_id'=> $session->payment_intent ?? null,
            'boost_amount'           => ($session->amount_total ?? 0) / 100,
            'boost_paid_at'          => current_time('mysql'),
        ], ['id' => $listing->id]);
    }

    bod_create_payment_record($owner_id, $listing->id ?? null, 'boost_' . $boost_type, $session);
